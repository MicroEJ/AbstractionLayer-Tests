...
This folder contains sources and projects to test drivers and implementation of print, time base, RAM, Core (see [Tests Description](#tests-description)), and MicroEJ Core (see [MicroEJ Core Validation](#microej-core-validation)).
...
dedicated section. See [Quick Start](#quick-start) section which summarize how to configure the
tests, how to launch them and how to analyze the report.

## Requirements

- Follow the [main Readme](../../README.rst).
- EEMBC Coremark
...
7. In the SDK, import the MicroEJ project `java-testsuite-runner-core` from the folder `tests/core`.
8. Follow `MicroEJ Core Validation Readme <java-testsuite-runner-core/README.rst>`_ and build this Application against the VEE Port to qualify.
9. Build the BSP and link it with the VEE Port runtime library and Application.
...
Follow the MicroEJ Core Validation [README](https://github.com/MicroEJ/Tool-Project-Template-VEEPort/blob/master/vee-port/validation/core/README.md) to build and link this Application against the VEE Port to qualify.

**Expected results**

No error must be thrown when executing this test. A typical execution trace is described in the MicroEJ Core Validation [README](https://github.com/MicroEJ/Tool-Project-Template-VEEPort/blob/master/vee-port/validation/core/README.md)

...
static void T_LLKERNEL_CHECK_resource_allocate(void) {
    ...
    do {
        ...
        if(((((uint8_t *)romAddr) <= (uint8_t *)(ramAddr)) && (((uint8_t *)romAddr + FEATURE_DATA_BUFFER_SIZE) > (uint8_t *)(ramAddr))) ||
        		((((uint8_t *)ramAddr) <= (uint8_t *)(romAddr)) && (((uint8_t *)ramAddr + FEATURE_DATA_BUFFER_SIZE) > (uint8_t *)(romAddr)))){
...
static void T_LLKERNEL_CHECK_resource_allocate_and_free(void)
{
    ...
    for(int32_t i = 0; i < NUMBER_OF_FEATURES; i++) {
        ...
        void *featureRomAddr = LLKERNEL_IMPL_getFeatureAddressROM__I(featureHandles[i]);
        TEST_ASSERT_NOT_NULL(featureRomAddr);

        void *featureRamAddr = LLKERNEL_IMPL_getFeatureAddressRAM__I(featureHandles[i]);
...
static void T_LLKERNEL_CHECK_resource_install_overflow(void)
{
    ...
    void *featureRomAddr = LLKERNEL_IMPL_getFeatureAddressROM__I(featureHandles[0]);
    TEST_ASSERT_NOT_NULL(featureRomAddr);

	/***********************************************************************
	* Test to verify that the kernel feature size is checked during a copy
	************************************************************************/
	...
	if(LLKERNEL_ERROR != result){
		// In the case of an asynchronous Copy to ROM, the function may return LLKERNEL_OK while the copy is on-going
		// on another task. The end of this asynchronus operation is checked using LLKERNEL_IMPL_flushCopyToROM().
		result = LLKERNEL_IMPL_flushCopyToROM();
	}
	...
	/*****************************************************************************
	* Test to verify that a ROM copy that overlaps two feature slots is forbidden
	******************************************************************************/

	// Try to allocate a second feature.
	featureHandles[1] = LLKERNEL_IMPL_allocateFeature__II(FEATURE_DATA_BUFFER_SIZE, FEATURE_DATA_BUFFER_SIZE);

	// Check if we could install more the second feature, otherwise the test is not relevant.
	if (0 != featureHandles[1]) {
		// Get the ROM start address of the next feature (index 1).
		void *nextFeatureRomAddr = LLKERNEL_IMPL_getFeatureAddressROM__I(LLKERNEL_IMPL_getFeatureHandle__I(1));
		// Attempt to copy in ROM data of the feature index 0 that overlaps feature slot at index 1.
		result = LLKERNEL_IMPL_copyToROM__LiceTea_lang_Ram_2LiceTea_lang_Ram_2I((void*)((uint8_t *)nextFeatureRomAddr - (FEATURE_DATA_BUFFER_SIZE / 2)), featureDataBuffer, (FEATURE_DATA_BUFFER_SIZE - 1));
		if(LLKERNEL_ERROR != result){
			// In the case of an asynchronous Copy to ROM, the function may return LLKERNEL_OK while the copy is on-going
			// on another task. The end of this asynchronus operation is checked using LLKERNEL_IMPL_flushCopyToROM().
			result = LLKERNEL_IMPL_flushCopyToROM();
...
static void T_LLKERNEL_CHECK_resource_install_out_of_bounds(void)
{
    ...
    // Try to copy a sequence of byte s outside ROM area (lower bounds)
    int32_t result = LLKERNEL_IMPL_copyToROM__LiceTea_lang_Ram_2LiceTea_lang_Ram_2I((void*)((uint8_t *)featureRomAddr - 0x1000), featureDataBuffer, FEATURE_DATA_BUFFER_SIZE);
	if(LLKERNEL_ERROR != result){
		// In the case of an asynchronous Copy to ROM, the function may return LLKERNEL_OK while the copy is on-going
		// on another task. The end of this asynchronus operation is checked using LLKERNEL_IMPL_flushCopyToROM().
		result = LLKERNEL_IMPL_flushCopyToROM();
